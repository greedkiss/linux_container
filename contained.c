...
int capabilities()
{
    ...
    for(size_t i = 0; i < num_caps; i++){
        if(prctl(PR_CAPBSET_DROP, drop_caps[i], 0, 0, 0, 0)){
...
int mounts(struct child_config *config)
{
    ...
    if(!mkdtemp(inner_mount_dir)){
        fprintf(stderr, "failed makeing the inner directory");
        return -1;
    }
    fprintf(stderr, "done.\n");
...
int resources(struct child_config *config)
{
    ...
    for(struct cgrp_control **cgrp = cgrps; *cgrp; cgrp++){
        ...
        if(mkdir(dir, S_IRUSR | S_IWUSR | S_IXUSR)){
            fprintf(stderr, "mkdir %s error", dir);
            return -1;
        }
        ...
        for(struct cgrp_setting **setting = (*cgrp)->settings; *setting; setting++){
            ...
            if((fd = open(path, O_WRONLY)) == -1){
                fprintf(stderr, "opening %s failed", path);
                return -1;
            }

            if(write(fd, (*setting)->value, sizeof((*setting)->value)) == -1){
                fprintf(stderr, "wrting to %s failed", path);
                close(fd);
                return -1;
            }
            close(fd);
...
int handle_child_uid_map(pid_t child_pid, int fd)
{
    ...
    if(has_userns){
        ...
        for(char **file = (char *[]){"uid_map", "gid_map", 0}; *file; file++){
            ...
            if(dprintf(uid_map, "0 %d %d", USERNS_OFFEST, USERNS_COUNT) == -1){
...
int choose_hostname(char *buf, size_t len)
{
    ...
    size_t ix = now.tv_nsec % 78;
		...
		snprintf(buf, len, "%05lx-%s", now.tv_sec, major[ix]);
	} else {
...
int main(int argc, char **argv)
{
    ...
    //namesapces
    if(sockerpair(AF_LOCAL, SOCK_SEQPACKET, 0, sockets)){
    ...
    int flags = CLONE_NEWNS | CLONE_NEWCGROUP | CLONE_NEWIPC | CLONE_NEWNET | CLONE_NEWPID | CLONE_NEWUTS;
